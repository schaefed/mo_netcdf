program test_putdata
  ...
  ! integer(i32)              :: val, x(nx), y(ny), time(ntime), tstep(nx, ny), tsteps(nx, ny, ntime)
  ! integer(i32)              :: val, x(:), y(:), time(:), tstep(:), tsteps(:)
  integer(i32)              :: val
  ...
  call var%putData(write1d, start=[1,1,3])
  ...
  call var%putData(write1d, start=[3,1,6], cnt=[1,ny])
  call var%getData(read1d, start=[3,1,6], cnt=[1,ny])
  call assertEqual(read1d, write1d, "Failed to set values of a 1D array")

  ! ! set and get the all values for sixth column, fourth row and every second timestep
  ! !! (4,6,1::2)
  write1d = full(ntime, 168_i32)
  call var%putData(write1d, start=[6,4], cnt=[1,1,ntime], stride=[1,1,2] )
  ...
  call var%putData(write2d, start=[1,1,4])
  call var%getData(read2d, start=[1,1,4], cnt=[nx,ny])
  ...
  call var%putData(write3d, cnt=[nx,ny,ntime], stride=[1,1,2] )
